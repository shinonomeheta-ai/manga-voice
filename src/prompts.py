...
# 掛け合い(会話の自然さ)
- emotion は**直前のセリフを受けた反応**として決める。ボケには驚き/呆れ、ツッコミは
  鋭く短く、問いには同じ熱量の返し、というように会話の流れを emotion に反映する。
- テンポを意識する: ツッコミ・即答・驚きは短く歯切れよく(delivery_note に "quick" 等)、
  ためや余韻は delivery_note に "pause" を記す(後段の音声化で間に反映される)。
- コマに**反応の絵だけがあってセリフが無い**場合(汗・口を開く・吹き出しの「！？」等)に限り、
  ごく短い相づち/感嘆を1つ足してよい("えっ" "うわっ" や [gasps]/[laughs])。
  原文に無いセリフを大量に創作してはならない(あくまで絵が示す反応の補完)。
