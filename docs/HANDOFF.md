...
> まず [/CLAUDE.md](../CLAUDE.md) を読み、次にここで現状を把握する。最終更新: 2026-06-23。

## ▶ 次にやること（NEXT）— 「次なにすべき？」はまずここを即答
- **いま着手**: **自動シナリオ生成を詰める**。`scenario_agent.py` を“作品を理解した”生成器にする。
  - 現状: 汎用(premise→scenario.json)で、作品設定(コンセプト/キャラ/パチンコ知識/出力形式/鉄則)を
    読み込まず、7パート構成も強制していない。`/scenario` スキル(対話型)だけが7パートネームを作れる。
  - 目標: 題材(テーマ名 or `資料/*.md`)＋作品設定 → **7パート構成のネーム指示書**を自動生成
    （`pipeline --neme` で音声化に繋がる形）。入力/出力形式/動かす場所(CLI/Web)は要相談。
- **済(音声生成・Webアプリ強化)**: Streamlit Cloud 稼働中。文字起こしHaiku化＋タイムアウト、
  プログレス/失敗段階表示、画像413対策(縮小+分割)、話者ゆるマッチ(敬称差)、プロジェクト保存=
  GitHub(dataブランチ)＋ローカルZIP、キャラ/ボイスID編集パネル＋**既定キャスト永続化(cast.json)**、
  全ブロック一括生成＋一括DL、ブロック並べ替え(⬆️⬇️)、取り込みモーダル(＋ボタン)、
  ブロック単位の安定性上書き、一覧キャッシュでラグ解消、スマホ対応。
- **課金の安全確認**: ElevenLabs の **Auto Top Up / 使用量課金を OFF**。APIキーのクレジット上限も設定済(3万)。
